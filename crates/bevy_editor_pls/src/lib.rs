...
pub fn spawn_default_windows(mut commands: Commands, mut tree: ResMut<EditorTabs>) {
    ...
    let parent = commands
        .spawn((
            Name::new("Editor Windows"),
            EditorWindowsCollection,
            Transform::default(), /*Transform sync breaks if parent doesn't have transform*/
        ))
        .id();
    ...
    let [top, bottom] = tree.state.split(
        ...
        0.25,
        egui_dock::Node::leaf_with(vec![nav.into(), h.into(), r.into(), a.into()]),
    );
