...
use bevy_egui::egui::{
    emath::Numeric, FontId, Frame, Key, KeyboardShortcut, Margin, Modifiers, Response, RichText, Sense, Stroke, Ui, Vec2, WidgetText
};
        ...
        fn ui(&self, world: &mut World, cx: EditorWindowContext, ui: &mut egui::Ui) {
            ...
            if let Some(e) = entities.iter().next() {
                if let Some(e) = draw_explorer(world, e, ui){
                    cx.get_mut::<HierarchyState>(world).unwrap().selected.entities = vec![e];
                }
            };
...
fn draw_explorer(world: &mut World, entity: Entity, ui: &mut Ui) -> Option<Entity>{
    ...
    let bundle_index = bundle.iter().enumerate().find(|a| *a.1 == entity).unwrap().0;//TODO cleaner 
    ...
    let siblings: Vec<EntityBundle> = match parent {
        Some(parent) => {
            ...
            index = children.iter().enumerate().find(|a| *a.1 == bundle[0]).unwrap().0;//TODO cleaner 
